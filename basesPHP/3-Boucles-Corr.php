...
<?php
$nombreEtoiles=20;
for($compteur=0;$compteur<$nombreEtoiles;$compteur++){
  echo '*';
}
 ?>
 ...
 for($compteur=0;$compteur<10;$compteur++){
  ...
  for($ligne=0;$ligne<4;$ligne++){
   ...
   <pre>
     ...
     nombre <- 5;
     Entier ligne,compteur;
     Pour(ligne de 0 à nombre non compris pas de 1)
       Pour(compteur de 0 à nombre non compris pas de 1)
         Afficher '*';
       Fin Pour
       Afficher 'saut de ligne';
     Fin Pour
   ...
   for($ligne=0;$ligne<$nombre;$ligne++){
    ...
    <pre>
      ...
      Pour(ligne de 0 à nombre non compris pas de 1)
        Pour(compteur de 0 à nombre non compris pas de 1)
          Si( (ligne >0) ET (ligne <nombre-1) ET (compteur > 0) ET (compteur < nombre-1) )
          alors
            Afficher ' '; // espace
          Sinon
            Afficher '*';
    ...
    for($ligne=0;$ligne<$nombre;$ligne++){
      for($compteur=0;$compteur<$nombre;$compteur++){
        if( ($ligne>0) && ($ligne <$nombre-1) && ($compteur>0) && ($compteur<$nombre-1) ){
          echo '  ';
        }
        else {
          echo '*';
     ...
     <pre>
       ...
       Entier ligne,compteur;
       Pour(ligne de 0 à nombre non compris pas de 1)
         Pour(compteur de 0 à ligne pas de 1)
           Afficher '*';
         Fin Pour
         Afficher 'saut de ligne';
       Fin Pour
